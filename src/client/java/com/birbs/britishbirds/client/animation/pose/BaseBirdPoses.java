 ...
 *
 * <p>Values are loaded from JSON ({@code assets/britishbirds/poses/}) at startup
 * via {@link PoseLoader}. The hardcoded constants below serve as fallbacks in
 * case the JSON files are missing or malformed.
 *
...
public final class BaseBirdPoses {
    ...
    // The archetype name used for JSON lookup — base poses are shared across
    // all archetypes, so we use "passerine" as the canonical source.
    ...
    public static final PoseData PERCHED = PoseLoader.getOrDefault(ARCH, "perched",
            PoseData.builder("perched")
    ...
    public static final PoseData ALERT = PoseLoader.getOrDefault(ARCH, "alert",
            PoseData.builder("alert")
     ...
     */
    public static final PoseData FLYING_CRUISE = PoseLoader.getOrDefault(ARCH, "flying_cruise",
            PoseData.builder("flying_cruise")
                    ...
                    .joint(BirdSkeleton.L_UPPER_WING, 0f, 0f, -0.1f)
    ...
    public static final PoseData FLYING_TAKEOFF = PoseLoader.getOrDefault(ARCH, "flying_takeoff",
            PoseData.builder("flying_takeoff")
                    .joint(BirdSkeleton.CHEST,       1.2f, 0f, 0f)
    ...
    public static final PoseData FLYING_LAND = PoseLoader.getOrDefault(ARCH, "flying_land",
            PoseData.builder("flying_land")
    ...
    public static final PoseData TUCKED = PoseLoader.getOrDefault(ARCH, "tucked",
            PoseData.builder("tucked")
    ...
    public static final PoseData BEAK_OPEN = PoseLoader.getOrDefault(ARCH, "beak_open",
            PoseData.builder("beak_open")
                    .joint(BirdSkeleton.LOWER_BEAK, 0.4f, 0f, 0f)
                    .build());

    /** Legs tucked for flight — extreme tuck values for in-flight leg retraction. */
    public static final PoseData LEGS_TUCKED = PoseLoader.getOrDefault(ARCH, "legs_tucked",
            PoseData.builder("legs_tucked")
     ...
     */
    public static final CyclicAnimation WINGBEAT = PoseLoader.getCyclicOrDefault(ARCH, "wingbeat",
            new CyclicAnimation(
                    "wingbeat",
    ...
    public static final CyclicAnimation WALK_CYCLE = PoseLoader.getCyclicOrDefault(ARCH, "walk_cycle",
            new CyclicAnimation(
                    "walk_cycle",
